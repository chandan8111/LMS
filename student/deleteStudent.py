...
class studentWindow:
    def __init__(self):
        ...
        # Title of the window
        self.win.title(" DELETE STUDENT DATA | LIBRARY MANAGEMENT SYSTEM |")
    def addframe(self):
        ...
        self.but = Button(self.frame, text='Delete Data >>', width=18, bg='light grey', fg='black',
                          font=("Poppins", 12, " bold"), command=self.delstudent)
    ...
    def delstudent(self):
        ...
        if ask == 'yes':
            try:
                conn = mysql.connector.connect(**config)
                cursor=conn.cursor()
                cursor.execute(f"DELETE FROM student WHERE id={ite}")
                conn.commit()
                conn.close()
                mbx.showinfo("Sussfull","Selected student has been deleted from database.")
                self.addframe()
            except:
                mbx.showwarning("Server Error","Please Try Again After Restart")
